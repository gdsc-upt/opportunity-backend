...
class AuthenticationBackend(ModelBackend):
    def authenticate(self, request, **credentials) -> Optional[User]:
        user = User.objects.get(email__iexact=credentials["email"])
        if self._check_password(user, credentials["password"]):
            return user
        return None

    def _check_password(self, user: User, password: AnyStr) -> bool:
        if ret := user.check_password(password):
    ...
    @classmethod
    def _stash_user(cls, user: Optional[User]) -> Optional[User]:
    ...
    @classmethod
    def unstash_authenticated_user(cls) -> Optional[User]:
